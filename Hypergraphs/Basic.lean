...
-- the intersection of two hyperedges, creating a new hyperedge that is active
-- only when both original hyperedges are active.
-- `and` defined as e_{and}(H) = e_1(H) ∩ e_2(H)

-- a union of hyperedges, where a new hyperedge is active if at least one of
-- the original hyperedges is active.
-- `or` defined as e_{or}(H) = e_1(H) ∪ e_2(H)

-- the combination of two hyperedges, `and` and `or`, is activated only when only
-- one of the original hyperedges is active.
-- `xor` defined as e_{xor}(H) = (e_1(H) ∪ e_2(H)) ∩ \overline{(e_1(H) ∩ e_2(H))}

-- inverting a hyperedge: a new hyperedge becomes active when the original one
-- is inactive.
-- `not` defined as e_{not}(H) = \overline{e(H)}

-- a mix of and and not operations, with the nand hyperedge active when the and
-- hyperedge is inactive.
-- `nand` defined as e_{nand}(H) = \overline{e_1(H) ∩ e_2(H)}

-- the union of or and not activates the nor hyperedge when the or hyperedge is
-- inactive.
-- `nor` defined as e_{nor}(H) = \overline{e_1(H) ∪ e_2(H)}

-- integration of xor and not operations, where the xnor hyperedge is active
-- when the xor hyperedge becomes inactive.
-- `xnor` defined as e_{xnor}(H) = \overline{(e_1(H) ∪ e_2(H)) ∩ \overline{(e_1(H) ∩ e_2(H))}}
