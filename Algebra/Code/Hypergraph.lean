...
--
--   For the worked hypergraph of V = {0,1,2,3},
--   edges e₁ = {0,1,2}, e₂ = {1,3}, and e₃ = {0,3}:
--   that is the 3-edge-row × 4-vertex-col table:
--
--           v₀ v₁ v₂ v₃
...
--
-- The [7,4,3] Hamming code (Algebra/Code/Hamming.lean, using its parity-check
-- matrix `H`) is a concrete 4-uniform hypergraph with minimum distance 3: it
-- demonstrates the uniqueness property with t = 1.
...
-- ============================================================================
--
-- The uniqueness theorem guarantees a low-weight solution is unique if it
-- exists. Finding it efficiently is a computational question. The structure
-- and axiom below capture the LPN assumption in the parity-check regime:
-- there exist k-uniform hypergraphs with minimum distance d whose
-- syndrome-decoding problem is computationally infeasible.

/-- A hypergraph suitable for cryptographic use: k-uniform, with minimum
distance d (so decoding is well-posed), and such that the syndrome-decoding
problem (find e of weight ≤ ⌊(d-1)/2⌋ from σ = H.syndrome e) is hard.
Hardness is a complexity-theoretic claim that cannot be formalized in Lean;
`SecureHypergraph` bundles the formalizable conditions and marks the
computational claim as an axiom. -/
structure SecureHypergraph (k d : ℕ) where
  H : Hypergraph ℕ
  uniform : H.IsUniform k
...
set_option maxRecDepth 1000000 in
example : ∀ c : Fin 7 → ZMod 2, H_matrix.mulVec c = 0 → c ≠ 0 → 3 ≤ hammingWeight c := by
  decide
