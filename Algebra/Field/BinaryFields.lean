...
-- ============================================================================
--
-- `GaloisField 2 n`: the unique GF(2ⁿ), constructed noncomputably as a splitting
-- field over 𝔽₂, so proofs apply but `native_decide` on elements does not.
...
--
-- For example, GF(2²) consists of the four elements {0,1,ω,ω+1} modulo
-- t²+t+1 = 0; in each one, doubling gives 0 because coefficients are in 𝔽₂:
--
--   0+0=0,   1+1=0,   ω+ω=0,   (ω+1)+(ω+1)=0.
