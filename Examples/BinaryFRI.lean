...
--   round 2:    verifier sends r₁ = α; prover folds g → a constant
...
example : ∀ x : G8, g (qmap α x) = foldW α r₀ cw x := by decide
...
example : foldW (α + 1) r₁ g 0 = foldW (α + 1) r₁ g (α + 1) := by decide
example : foldW (α + 1) r₁ g (α² + 1) = foldW (α + 1) r₁ g (α² + α) := by decide

-- The final word: g is itself a digit (degree 1 < 2), so its components
-- are the constants p₀ = α², p₁ = α+1, and the fold with r₁ = α is the
-- constant p₀ + r₁·p₁ = α. Two rounds folded degree 3 → 1 → 0; the
-- verifier reads one constant from the prover's last message.
example : foldW (α + 1) r₁ g 0 = α ∧
    foldW (α + 1) r₁ g (α² + 1) = α := by decide
