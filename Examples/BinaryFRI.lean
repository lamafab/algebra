...
--   round 1:    verifier sends c₀ = 1; prover folds word₀ → word₁ on q(L)
--               check: foldW α c₀ word₀ x = word₁ (qmap α x), any fiber
--   round 2:    verifier sends c₁ = α; prover folds word₁ → a constant
--               check: foldW (α+1) c₁ word₁ y = const, any fiber
...
the folded word's value at q(x) is p₀(y) + c·p₁(y) with slope
p₁(y) = (w(x) + w(x+β)) / β read off the fiber {x, x + β}.

(foldWord in BinaryFRI.lean §1b, redefined locally; inv β is 1/β). -/
def foldW (β c : G8) (w : G8 → G8) (x : G8) : G8 :=
  w x + (x + c) * (w x + w (x + β)) * inv β

/-- The round-1 challenge, sampled by the verifier after the commitment.
c₀ = 1 here; the chain needs it to be unpredictable, not any particular
value. -/
def c₀ : G8 := 1
...
example : foldW α c₀ word₀ 0 = foldW α c₀ word₀ α := by decide
example : foldW α c₀ word₀ 1 = foldW α c₀ word₀ (α + 1) := by decide
example : foldW α c₀ word₀ α² = foldW α c₀ word₀ (α² + α) := by decide
example : foldW α c₀ word₀ (α² + 1) = foldW α c₀ word₀ (α² + α + 1) := by decide

-- The verifier's fiber-local computation agrees with the prover's fold
-- at every point and every challenge: p₀(y) + c·p₁(y) with the fiber
-- coordinate replaced by c.
example : ∀ c x : G8, foldW α c word₀ x =
    (1 + α * qmap α x) + c * ((α² + 1) + qmap α x) := by decide

-- Round 1's output on the image {0, α+1, α²+1, α²+α}: with c₀ = 1 the
-- fold is p₀(t) + c₀·p₁(t) = α² + (α+1)t, the values α², 1, 0, α²+1
-- (in the order listed). The degree halved: 3 → 1.
example : foldW α c₀ word₀ 0 = α² ∧ foldW α c₀ word₀ 1 = 1 ∧
    foldW α c₀ word₀ α² = 0 ∧ foldW α c₀ word₀ (α² + 1) = α² + 1 := by decide
...
/-- The round-2 challenge c₁ = α, chosen independently of c₀ = 1. -/
def c₁ : G8 := α
...
example : ∀ x : G8, word₁ (qmap α x) = foldW α c₀ word₀ x := by decide
...
example : foldW (α + 1) c₁ word₁ 0 = foldW (α + 1) c₁ word₁ (α + 1) := by decide
example : foldW (α + 1) c₁ word₁ (α² + 1) = foldW (α + 1) c₁ word₁ (α² + α) := by decide

-- The final word: word₁ is itself a digit (degree 1 < 2), so its
-- components are the constants p₀ = α², p₁ = α+1, and the fold with
-- c₁ = α outputs one value: word₂, the constant α on the final domain
-- {0, α+1}. Two rounds folded degree 3 → 1 → 0; the verifier reads
-- word₂ outright.
--
-- TODO: Clarify this further
--
-- An evaluation claim m(r) = v at a point r outside the  domain is then opened
-- by the quotient opening (EvalOpening.lean).
...
example : ∀ y : G8, word₂ (qmap (α + 1) y) = foldW (α + 1) c₁ word₁ y := by decide

-- Telescoped back to word₀: at the doubly folded image point q₁(q₀(x)),
-- the outer fold of word₁ over the round-1 fiber of x.
example : ∀ x : G8,
    word₂ (qmap (α + 1) (qmap α x)) = foldW (α + 1) c₁ word₁ (qmap α x) := by decide
